...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("⚠️  Database unavailable. Running in memory-only mode.");
    ...
    private static void doctorMenu() {
        ...
        switch (choice) {
            ...
            case 3 -> {
                ...
                if (filtered.isEmpty()) { System.out.println("No doctors found."); return; }
    ...
    private static void reportMenu() {
        ...
        switch (choice) {
            ...
            case 2 -> {
                List<Appointment> all = manager.getAllAppointments();
                if (all.isEmpty()) { System.out.println("No appointments to generate receipt for."); return; }
    ...
    private static void databaseMenu() {
        if (!dbAvailable) {
            System.out.println("⚠️  Database is not connected. Check MySQL is running and schema.sql has been executed.");
            return;
        }
        ...
        switch (choice) {
            ...
            case 2 -> {
                try {
                    for (Doctor d : manager.getAllDoctors()) doctorDAO.insertDoctor(d);
                } catch (SQLException e) { System.out.println("❌ DB error: " + e.getMessage()); }
            }
            case 3 -> {
                try {
                    for (Appointment a : manager.getAllAppointments()) appointmentDAO.insertAppointment(a);
                } catch (SQLException e) { System.out.println("❌ DB error: " + e.getMessage()); }
            }
            case 4 -> {
            ...
            case 5 -> {
                try { patientDAO.printDatabaseInfo(); }
    ...
    private static void bookScheduled() {
        ...
        manager.getAllDoctors().forEach(d -> System.out.println("  " + d.getId() + " - Dr. " + d.getName() + " (" + d.getSpecialization() + ")"));
    ...
    private static void loadFromFiles() {
        ...
        try {
            ...
            if (!patients.isEmpty())     manager.loadPatients(patients);
            if (!doctors.isEmpty())      manager.loadDoctors(doctors);
            if (!appointments.isEmpty()) manager.loadAppointments(appointments);
    ...
    private static void seedDemoData() {
        System.out.println("\n📋 Loading demo data...");
        manager.registerDoctor(new Doctor("D001", "Aisha Kebede", "0911000001",
                "aisha@hospital.com", "Cardiology", "LIC-100"));
        manager.registerDoctor(new Doctor("D002", "Samuel Tesfaye", "0911000002",
                "samuel@hospital.com", "General Practice", "LIC-101"));
        manager.registerPatient(new Patient("P001", "Yonas Alemu", "0922111001",
                "yonas@mail.com", 34, "O+"));
        manager.registerPatient(new Patient("P002", "Marta Haile", "0922111002",
                "marta@mail.com", 27, "A-"));
        System.out.println("✅ Demo data ready. Try booking an appointment!\n");
    }
